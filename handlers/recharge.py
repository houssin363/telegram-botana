...
def register(bot, history):
    """تسجيل جميع هاندلرز الشحن (مستخدم + أدمن)."""
    ...
    def get_reference(msg):
        ...
        bot.send_message(
            msg.chat.id,
            "💰 أرسل مبلغ الشحن (بالليرة السورية):",
            reply_markup=keyboards.recharge_menu()
        )
    ...
    @bot.callback_query_handler(
        ...
        in ["user_confirm_recharge", "user_edit_recharge", "user_cancel_recharge"]
    )
        ...
        if call.data == "user_confirm_recharge":
            ...
            markup.add(
                types.InlineKeyboardButton(
                    "✅ قبول الشحن",
                    callback_data=f"confirm_add_{user_id}_{data['amount']}",
                ),
                types.InlineKeyboardButton(
                    "❌ رفض", callback_data=f"reject_add_{user_id}"
                ),
            )
